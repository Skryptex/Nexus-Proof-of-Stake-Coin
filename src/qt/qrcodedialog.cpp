...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NXS unit input in lnReqAmount, we generate the URI with NXS as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::NXS, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
